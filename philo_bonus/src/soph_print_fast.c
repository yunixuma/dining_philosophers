...
/*   Updated: 2023/01/13 11:13:02 by Yoshihiro K      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

void	soph_print_log(long ts, int i_proc, const char *msg, t_var *var)
{
	char	str[LEN_LOG];

	if ((var->param->max_turn == TURN_TERM \
		|| var->turn >= var->param->max_turn) \
		&& var->param->max_turn != TURN_INF)
		return ;
	ft_strlcpy_nbr(str, ts / 1000, LEN_TS);
	ft_strlcpy_nbr(str + ft_strlen(str), i_proc + 1, LEN_ID);
...
int	soph_print_err(int errnum)
{
	...
	else if (errnum == ERR_SEM)
		ft_putstr_fd(MSG_SEM, STDERR_FILENO);
